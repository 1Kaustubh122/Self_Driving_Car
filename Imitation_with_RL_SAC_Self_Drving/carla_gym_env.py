...
class CarlaEnv(gym.Env):
    def __init__(self, host='localhost', port=2000):
        ...
        self.observation_space = Dict({
            "image": Box(low=0, high=255, shape=(5, 360, 640), dtype=np.uint8),
            "state": Box(
    ...
    def get_observation(self):
        ...
        brake = control.brake
        state_obs = np.array([speed, steering, throttle, brake], dtype=np.float32)
        state_obs = np.clip(
            state_obs,
            a_min=[0.0, -1.0, 0.0, 0.0],
            a_max=[100.0, 1.0, 1.0, 1.0]
        )
